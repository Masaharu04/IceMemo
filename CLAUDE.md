...
- **状態管理**: `@Observable` マクロ（iOS 17+ の Observation フレームワーク）
  - ViewModel プロトコルは `AnyObject, Observable` に準拠
  - 具象クラスに `@Observable` を付与し、`@Published` は使用しない
  - バインディングが必要な View は `@Bindable var vm: VM` を使用
  - `@StateObject` / `@ObservedObject` / `@EnvironmentObject` は使用しない
  - Coordinator は `@State` で保持し、`.environment(coordinator)` で子 View に渡す
